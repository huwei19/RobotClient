...
[Serializable]
 public class Tab_AddSignAward
{ private const string TAB_FILE_DATA = "Table/AddSignAward.txt";
 enum _ID
 {
 INVLAID_INDEX=-1,
ID_ID,
...
public static bool LoadTable(Dictionary<int, List<Tab_AddSignAward> > _tab)
 {
 ...
 public static Tab_AddSignAward SerializableTable(string[] valuesList,int skey)
 {
 if ((int)_ID.MAX_RECORD!=valuesList.Length)
 {
 throw TableException.ErrorReader("Load {0} error as CodeSize:{1} not Equal DataSize:{2}", GetInstanceFile(),_ID.MAX_RECORD,valuesList.Length);
 }
 Tab_AddSignAward _values = new Tab_AddSignAward();
