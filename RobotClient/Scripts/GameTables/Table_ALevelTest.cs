...
[Serializable]
 public class Tab_ALevelTest
{ private const string TAB_FILE_DATA = "Table/A_Level_Test.txt";
 enum _ID
 {
 INVLAID_INDEX=-1,
ID_ID,
...
public int getParamIntCount() { return 20; } 
 ...
 if(idx>=0 && idx<20) return m_ParamInt[idx];
 return -1;
 }
 public ReadOnlyCollection <int> ParamInts { get { return Array.AsReadOnly(m_ParamInt);}} 

public int getParamStrCount() { return 20; } 
 private string[] m_ParamStr = new string[20];
 public string GetParamStrbyIndex(int idx) {
 if(idx>=0 && idx<20) return m_ParamStr[idx];
 return "";
 }
 public ReadOnlyCollection <string> ParamStrs { get { return Array.AsReadOnly(m_ParamStr);}} 

private int m_Reserved;
 public int Reserved { get{ return m_Reserved;}}

public static bool LoadTable(Dictionary<int, List<Tab_ALevelTest> > _tab)
 {
 ...
 public static Tab_ALevelTest SerializableTable(string[] valuesList,int skey)
 {
 if ((int)_ID.MAX_RECORD!=valuesList.Length)
 {
 throw TableException.ErrorReader("Load {0} error as CodeSize:{1} not Equal DataSize:{2}", GetInstanceFile(),_ID.MAX_RECORD,valuesList.Length);
 }
 Tab_ALevelTest _values = new Tab_ALevelTest();
